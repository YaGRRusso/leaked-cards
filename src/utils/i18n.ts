...
function getNestedValue(obj: unknown, key: string) {
  return key
    .split('.')
    .reduce(
      (acc, part) =>
...
function translate(
  ...
  if (typeof messageString === 'string') {
    try {
      ...
      if (!msg) {
        msg = new IntlMessageFormat(messageString, locale, undefined, {
          ignoreTag: true,
        });
      ...
      console.error(
        `Error formatting message key "${key}" for locale "${locale}":`,
        error
      );
